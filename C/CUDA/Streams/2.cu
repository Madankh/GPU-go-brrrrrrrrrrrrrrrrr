...
__global__ void kernerl2(float *data, int n){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if(idx<n){
        data[idx] += 1.0f;
    }
...
int main(void){
    ...
    // Asynchronous memory copy and kernel execution is stream1
    ...
    kernerl2<<<(N + 256)/ 256, 256 ,0, stream2>>>(d_data, N);
